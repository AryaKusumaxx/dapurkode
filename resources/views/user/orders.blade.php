...
<div class="bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p class="mt-2">Kelola dan pantau pesanan Anda</p>
    </div>
</div>
        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2">
                ...
                <li class="flex items-center">
                    ...
                    <span class="ml-2 text-gray-700 font-medium">Daftar Pesanan</span>
                </li>
            </ol>
        </nav>
    </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-lg font-medium text-gray-900">Semua Pesanan</h2>
            ...
            <div class="flex items-center space-x-2">
                ...
                <div class="relative">
                    <select id="date-filter" class="appearance-none block pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">
                                @if($order->items->count() > 0)
                                    ...
                                    <span class="text-xs text-gray-500">+{{ $order->items->count() - 1 }} produk lainnya</span>
                                    @endif
                                @else
                                    N/A
                                @endif
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            ...
                            <a href="{{ route('payment.show', $order->invoice) }}" class="text-indigo-600 hover:text-indigo-900">Bayar</a>
                            @endif
                        </td>
                    </tr>
                    @empty
...
<script>
    document.getElementById('status-filter').addEventListener('change', function() {
        window.location.href = "{{ route('user.orders') }}?status=" + this.value;
    });
    
    document.getElementById('date-filter').addEventListener('change', function() {
        window.location.href = "{{ route('user.orders') }}?period=" + this.value;
    });
    ...
    if (urlParams.has('period')) {
        document.getElementById('date-filter').value = urlParams.get('period');
    }
</script>
@endpush
@endsection
